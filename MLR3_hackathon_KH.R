...
#Using tidyverse
model <- lm(sale_price ~ lot_area + lot_config + neighborhood + bldg_type +
              overall_qual + overall_cond + year_built, data = ames)
summary(model)$coefficient

#Overall quality is the most significant factor with lot area as number 2
#Rerun without Neighborhood and condition
refined <- lm(sale_price ~ lot_area + overall_qual + year_built, 
              data = ames)
summary(refined)

# choose subset of variables - lot area, lot cofig, neighborhood, building type,
# overall quality, overall condition, year built & outcome of sale price
ames2 <- ames %>%
  select(
    order,
    lot_area,
...
#create the task 
task = as_task_regr(ames2, target = 'sale_price', id = 'order')

#train and test set split 
set.seed(7)
#use two thirds for training
train_set = sample(ames2$order, 0.67 * nrow(ames2))

#remaining third for test
test_set = setdiff(ames2$order, train_set)
...
# plot regression tree - is it picking out the most important variables? 
# Only overall quality and lot area are showing up on the tree
rpart.plot(learner$model, digits = -2)
